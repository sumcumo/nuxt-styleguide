...
module.exports = {
	...
	srcDir: __dirname,
	modules: [
		[ '@sum.cumo/nuxt-markdown-component-loader', { loader: { exclude: path.resolve(__dirname, 'docs') } } ],
		[
			'@sum.cumo/nuxt-styleguide',
			{
				styleguideData: {
					repositoryHomepage: 'https://github.com/sumcumo/nuxt-styleguide/blob/master'
				},
				name: pkg.name
			}
		]
	...
	markdownComponents: {
		...
		h2: {
			...
			props($el) {
				return {
					level: 2,
					anchor: $el.text().replace(/\W+/g, '-').toLowerCase().replace(/^-/, '').replace(/-$/, '')
				};
			}
		},
		a: '~/components/A'
